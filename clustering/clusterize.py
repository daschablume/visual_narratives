...
def prepare_df_to_clustering(df) -> tuple[list[dict], list[dict]]:
    '''
    Create two lists of dictionaries for verbs and noun phrases from the DataFrame
        to be used for clustering.
    Each dictionary contains the word, sentence_id, image_id, and position_idx.
    '''
    verbs_meta = []
    np_meta = []
...
def replace_with_clusterized_labels(
    ...
    # i.e. finish replace_with_clusterized_labels_altern()
...
def replace_with_clusterized_labels_altern(clusters_df, sentences_df):
    '''
    I want to replace the labels with clusterized labels using pd methods (not iterrows()).
    '''
